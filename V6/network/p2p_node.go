...
// TransactionValidator interface para validação (mantendo a versão completa)
// Stub para log de segurança
func logSecurityEvent(eventType, from, message, severity string, resolved bool) {
	fmt.Printf("🔒 [SECURITY] %s from %s: %s (severity: %s, resolved: %v)\n", eventType, from, message, severity, resolved)
}
