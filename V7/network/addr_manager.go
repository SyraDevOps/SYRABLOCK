...
func (am *AddrManager) saveAddresses() {
	...
	if err != nil {
		fmt.Printf("❌ Erro ao criar arquivo temporário de peers: %v\n", err)
		return
	}
	...
	if err := encoder.Encode(addresses); err != nil {
		fmt.Printf("❌ Erro ao salvar peers: %v\n", err)
		file.Close()
		return
	}
	...
	if err := os.Rename(tempFile, am.filePath); err != nil {
		fmt.Printf("❌ Erro ao finalizar salvamento de peers: %v\n", err)
		return
	}
}
...
func (am *AddrManager) GetAddresses(max int, includeNew, includeTried bool) []*KnownAddress {
	...
	addFromMap := func(addresses map[string]*KnownAddress, count int) int {
		...
		for key, addr := range addresses {
			...
			// Pula endereços tentados muito recentemente (evita spam)
			if now.Sub(addr.LastAttempt) < time.Minute {
				continue
			}
	...
	if includeTried {
		addFromMap(am.triedAddresses, max)
	}

	// Depois, se precisar, adiciona novos endereços
	if includeNew && len(result) < max {
		addFromMap(am.newAddresses, max-len(result))
	}
...
func (am *AddrManager) CleanupAddresses() {
	...
	for key, addr := range am.knownAddresses {
		if addr.ConnectSuccess == 0 && addr.LastSeen.Before(staleThreshold) {
